...
pool10000 <- rbind(params_risque_eleve,
                   params_data[sample(seq(total_pool_size), 9999),])
sum(pool10000$lambda * (pool10000$alpha / pool10000$beta))


pool10_contrib <- compute_cm(2^15, 1, pool10, "unbiased")
pool100_contrib <- compute_cm(2^18, 1, pool100, "unbiased")
pool1000_contrib <- compute_cm(2^20, 1, pool1000, "unbiased")
pool10000_contrib <- compute_cm(2^20, 5, pool10000, "unbiased")
